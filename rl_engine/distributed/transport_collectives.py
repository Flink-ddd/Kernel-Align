...
class TorchDistributedDeterministicCollective:
    ...
    def all_gather(
        ...
        *,
        ...
        with self._lock:
            ...
            rank_inputs = self._all_gather_transport(input)
            out.copy_(rank_inputs.reshape(output_shape))
        return out
    ...
    def close(self) -> None:
        ...
        The AllGather implementation owns no persistent device allocation, so
        closing is an idempotent lifecycle boundary rather than a transport
    ...
    def _all_gather_transport(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.world_size == 1:
            return input.clone().reshape((1, *input.shape))
        input_flat = input.view(-1)
        gathered_flat = torch.empty(
            self.world_size * input_flat.numel(),
            dtype=input.dtype,
            device=input.device,
        )
