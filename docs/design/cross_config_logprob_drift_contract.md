...
and reports structured issues without creating engines.
...
- supported target tags, devices, dtypes, and topology values;
...
`COMPLETE` is published last. Resume accepts only a complete attempt whose case,
identity, materialization, scorer, operator, environment, and tensor artifacts
